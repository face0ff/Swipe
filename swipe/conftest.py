...
def django_db_setup(django_db_setup, django_db_blocker):
    with django_db_blocker.unblock():
        admin = User.objects.create_superuser(email='admin@example.com', username='admin@example.com',password='string', role='manager')
        EmailAddress.objects.create(email=admin.email, verified=True, primary=True, user_id=admin.id)
        user = User.objects.create_user(email='user@example.com', username='user@example.com', password='string', role='user')
        EmailAddress.objects.create(email=user.email, verified=True, primary=True, user_id=user.id)
        owner = User.objects.create_user(email='owner@example.com', username='owner@example.com', password='string', role='owner')
